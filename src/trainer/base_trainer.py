...
class WikiTextDataset(torch.utils.data.Dataset):
    def __init__(self, split: str, seq_length: int, tokenizer):
        ...
        for i in tqdm(range(0, len(texts), batch_size), desc="Tokenizing"):
            ...
            if len(all_tokens) > 5_000_000 and split == "train":
                print(f"  Using first 5M tokens (memory constraint)")
                break
...
class BaseTrainer:
    ...
    def _setup_model(self):
        ...
        self.model = GPT2LMHeadModel(gpt_config).to(self.device)
    ...
    def _setup_data(self):
        ...
        train_ds = WikiTextDataset("train", seq_len, self.tokenizer)
        val_ds = WikiTextDataset("validation", seq_len, self.tokenizer)
    ...
    def save_checkpoint(self, step: int, loss: float):
        ...
        path = save_dir / f"checkpoint_step_{step}.pt"
        torch.save({
            "step": step,
